...
class Train
  ...
  def forward
    @current_location += 1 if @route.stations[@current_location] != @route.finish_station
  end

  #Пермещается на одну станцию назад по маршруту
  def back
    @current_location -= 1 if @route.stations[@current_location] != @route.start_station
  end
  ...
  def previos_station
    if  @route.stations[@current_location] != @route.start_station
  ...
  def next_station
    if @route.stations[@current_location] != @route.finish_station
      @route.stations[@current_location +1]
    else
