...
=end
  ...
  def get_route
    ...
    if (train < 1 || train > trains.size) || (route < 1 || route > routes.size)
      ...
      #puts "First station of route #{station}"
      stations.each do |x| 
        if x.name == station && !(x.trains.include?(trains[train-1]))
          x.trains << trains[train-1]
        end
      end
      #puts "Поезд #{trains[train-1]} помещен на первую станцию маршрута #{station}"
    end
  end
  ...
  def add_wagon
    if trains.empty?
      ...
      trains[train-1].push_wagon(wagon) if (train > 0 && train <= trains.size)
    end
  end
